...
public final class Twofish {
  ...
  public static synchronized Object makeKey(byte[] k) {
    ...
    // разделяем ключ на четные и нечетные 32-битные элементы
    int i, j, offset = 0;
    for (i = 0, j = k64Cnt - 1; i < 4 && offset < length; i++, j--) {
      ...
      k32o[i] = (k[offset++] & 0xFF) | (k[offset++] & 0xFF) << 8
                | (k[offset++] & 0xFF) << 16 | (k[offset++] & 0xFF) << 24;
      sBoxKey[j] = RS_MDS_Encode(k32e[i], k32o[i]); // обратный порядок
    }
    // вычисляем подэелменты ключа для PHT.
    int q, A, B;
  ...
  /**
   * Шифорование одного блока текста
   *
   * @param in         текст
   * @param inOffset   Индекс, с которого следует начинать читать данные.
   * @param sessionKey ключ
  ...
  /**
   * Расшифровать один блок текста
   *
   * @param in         Зашифрованный текст
   * @param inOffset   Индекс, с которого следует начинать читать данные.
   * @param sessionKey Ключ
  ...
  /**
   * Использование (12, 8) Reed-Solomon поверх GF(256)
   *
   * @param k0 первый 32-bit блок.
   * @param k1 второй 32-bit блок.
   * @return Сгенерированный RS код
   */
  ...
  /**
   * @return True если массивы имеют идентичное содержание
   */
